...
`.github/workflows/ci.yml` (ticket A5) runs on every pull request targeting `master`: `npm run lint` (ESLint via `expo lint`, config auto-generated by that command), `npm run typecheck`, then `npm test` (the full A4 suite, browsers installed via `npx playwright install --with-deps chromium` first). A branch ruleset on `master` requires that check to pass before a PR can be merged — there is no way to land a change with a failing lint, type, unit, token, or e2e/axe step.
