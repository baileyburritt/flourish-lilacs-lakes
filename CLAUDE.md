...
From `app/`, `npm test` (ticket A4) runs three suites in order — unit (Jest), token-conformance, then end-to-end (Playwright with `axe-core` scoped to WCAG 2.1 A/AA tags, run against the static `npm run build:web` output) — and stops at the first failure. `scripts/check-token-conformance.js` fails on any hex color literal found in `app/` source outside a future `theme/tokens.ts`; until ticket C1 generates that file, no hex literal is allowed anywhere in `app/`. The axe scan is intentionally scoped to `wcag2a`/`wcag2aa`/`wcag21a`/`wcag21aa` tags rather than axe's full default ruleset, which also includes best-practice rules (e.g. requiring a `<main>` landmark) beyond this project's stated WCAG 2.1 AA bar.
