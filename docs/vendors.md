...
| Fly.io | Hosting for the `server/` Fastify API (F4) | _chosen 2026-09-13; account not yet created_ | — | — | Full API outage — no gem/trip/bookmark endpoints, no auth verification round-trip. |
| Cloudflare Pages | Hosting for the Expo web static export (F4) | _chosen 2026-09-13; likely reuses the Cloudflare account already used for R2 — confirm_ | — | — | Web PWA unreachable; native iOS/Android unaffected. |
...
- The two remaining "not yet provisioned" rows (hosting: Fly.io, Cloudflare
  Pages) still need an account created by the human before F4 can start — an
  agent does not create these on its own initiative.
